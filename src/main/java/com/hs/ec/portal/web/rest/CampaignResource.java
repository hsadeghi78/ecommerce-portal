...
/**
 * REST controller for managing {@link com.hs.ec.portal.domain.Campaign}.
 */
@RestController
...
public class CampaignResource {
    ...
    public Mono<ResponseEntity<List<CampaignDTO>>> getAllCampaigns(
        ...
        return campaignService
            ...
            .zipWith(campaignService.findByCriteria(criteria, pageable).collectList())
     ...
     */
    @GetMapping("/count")
    public Mono<ResponseEntity<Long>> countCampaigns(CampaignCriteria criteria) {
        log.debug("REST request to count Campaigns by criteria: {}", criteria);
        return campaignService.countByCriteria(criteria).map(count -> ResponseEntity.status(HttpStatus.OK).body(count));
    }
