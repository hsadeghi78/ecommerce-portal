...
import classTypeResolve from './class-type-routing-resolve.service';

describe('ClassType routing resolve service', () => {
  let mockRouter: Router;
  let mockActivatedRouteSnapshot: ActivatedRouteSnapshot;
  let service: ClassTypeService;
  let resultClassType: IClassType | null | undefined;
  ...
  describe('resolve', () => {
    it('should return IClassType returned by find', () => {
      ...
      TestBed.runInInjectionContext(() => {
        classTypeResolve(mockActivatedRouteSnapshot).subscribe({
          next(result) {
    ...
    it('should return null if id is not provided', () => {
      ...
      expect(resultClassType).toEqual(null);
    });

    it('should route to 404 page if data not found in server', () => {
      // GIVEN
      jest.spyOn(service, 'find').mockReturnValue(of(new HttpResponse<IClassType>({ body: null })));
      mockActivatedRouteSnapshot.params = { id: 123 };
