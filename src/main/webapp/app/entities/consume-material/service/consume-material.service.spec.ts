import { TestBed } from '@angular/core/testing';
import { HttpClientTestingModule, HttpTestingController } from '@angular/common/http/testing';

import { IConsumeMaterial } from '../consume-material.model';
import { sampleWithRequiredData, sampleWithNewData, sampleWithPartialData, sampleWithFullData } from '../consume-material.test-samples';

import { ConsumeMaterialService } from './consume-material.service';

const requireRestSample: IConsumeMaterial = {
  ...sampleWithRequiredData,
};

describe('ConsumeMaterial Service', () => {
  let service: ConsumeMaterialService;
  let httpMock: HttpTestingController;
  let expectedResult: IConsumeMaterial | IConsumeMaterial[] | boolean | null;

  beforeEach(() => {
    TestBed.configureTestingModule({
      imports: [HttpClientTestingModule],
    });
    expectedResult = null;
    service = TestBed.inject(ConsumeMaterialService);
    httpMock = TestBed.inject(HttpTestingController);
  });

  describe('Service methods', () => {
    it('should find an element', () => {
      const returnedFromService = { ...requireRestSample };
      const expected = { ...sampleWithRequiredData };

      service.find(123).subscribe(resp => (expectedResult = resp.body));

      const req = httpMock.expectOne({ method: 'GET' });
      req.flush(returnedFromService);
      expect(expectedResult).toMatchObject(expected);
    });

    it('should create a ConsumeMaterial', () => {
      const consumeMaterial = { ...sampleWithNewData };
      const returnedFromService = { ...requireRestSample };
      const expected = { ...sampleWithRequiredData };

      service.create(consumeMaterial).subscribe(resp => (expectedResult = resp.body));

      const req = httpMock.expectOne({ method: 'POST' });
      req.flush(returnedFromService);
      expect(expectedResult).toMatchObject(expected);
    });

    it('should update a ConsumeMaterial', () => {
      const consumeMaterial = { ...sampleWithRequiredData };
      const returnedFromService = { ...requireRestSample };
      const expected = { ...sampleWithRequiredData };

      service.update(consumeMaterial).subscribe(resp => (expectedResult = resp.body));

      const req = httpMock.expectOne({ method: 'PUT' });
      req.flush(returnedFromService);
      expect(expectedResult).toMatchObject(expected);
    });

    it('should partial update a ConsumeMaterial', () => {
      const patchObject = { ...sampleWithPartialData };
      const returnedFromService = { ...requireRestSample };
      const expected = { ...sampleWithRequiredData };

      service.partialUpdate(patchObject).subscribe(resp => (expectedResult = resp.body));

      const req = httpMock.expectOne({ method: 'PATCH' });
      req.flush(returnedFromService);
      expect(expectedResult).toMatchObject(expected);
    });

    it('should return a list of ConsumeMaterial', () => {
      const returnedFromService = { ...requireRestSample };

      const expected = { ...sampleWithRequiredData };

      service.query().subscribe(resp => (expectedResult = resp.body));

      const req = httpMock.expectOne({ method: 'GET' });
      req.flush([returnedFromService]);
      httpMock.verify();
      expect(expectedResult).toMatchObject([expected]);
    });

    it('should delete a ConsumeMaterial', () => {
      const expected = true;

      service.delete(123).subscribe(resp => (expectedResult = resp.ok));

      const req = httpMock.expectOne({ method: 'DELETE' });
      req.flush({ status: 200 });
      expect(expectedResult).toBe(expected);
    });

    describe('addConsumeMaterialToCollectionIfMissing', () => {
      it('should add a ConsumeMaterial to an empty array', () => {
        const consumeMaterial: IConsumeMaterial = sampleWithRequiredData;
        expectedResult = service.addConsumeMaterialToCollectionIfMissing([], consumeMaterial);
        expect(expectedResult).toHaveLength(1);
        expect(expectedResult).toContain(consumeMaterial);
      });

      it('should not add a ConsumeMaterial to an array that contains it', () => {
        const consumeMaterial: IConsumeMaterial = sampleWithRequiredData;
        const consumeMaterialCollection: IConsumeMaterial[] = [
          {
            ...consumeMaterial,
          },
          sampleWithPartialData,
        ];
        expectedResult = service.addConsumeMaterialToCollectionIfMissing(consumeMaterialCollection, consumeMaterial);
        expect(expectedResult).toHaveLength(2);
      });

      it("should add a ConsumeMaterial to an array that doesn't contain it", () => {
        const consumeMaterial: IConsumeMaterial = sampleWithRequiredData;
        const consumeMaterialCollection: IConsumeMaterial[] = [sampleWithPartialData];
        expectedResult = service.addConsumeMaterialToCollectionIfMissing(consumeMaterialCollection, consumeMaterial);
        expect(expectedResult).toHaveLength(2);
        expect(expectedResult).toContain(consumeMaterial);
      });

      it('should add only unique ConsumeMaterial to an array', () => {
        const consumeMaterialArray: IConsumeMaterial[] = [sampleWithRequiredData, sampleWithPartialData, sampleWithFullData];
        const consumeMaterialCollection: IConsumeMaterial[] = [sampleWithRequiredData];
        expectedResult = service.addConsumeMaterialToCollectionIfMissing(consumeMaterialCollection, ...consumeMaterialArray);
        expect(expectedResult).toHaveLength(3);
      });

      it('should accept varargs', () => {
        const consumeMaterial: IConsumeMaterial = sampleWithRequiredData;
        const consumeMaterial2: IConsumeMaterial = sampleWithPartialData;
        expectedResult = service.addConsumeMaterialToCollectionIfMissing([], consumeMaterial, consumeMaterial2);
        expect(expectedResult).toHaveLength(2);
        expect(expectedResult).toContain(consumeMaterial);
        expect(expectedResult).toContain(consumeMaterial2);
      });

      it('should accept null and undefined values', () => {
        const consumeMaterial: IConsumeMaterial = sampleWithRequiredData;
        expectedResult = service.addConsumeMaterialToCollectionIfMissing([], null, consumeMaterial, undefined);
        expect(expectedResult).toHaveLength(1);
        expect(expectedResult).toContain(consumeMaterial);
      });

      it('should return initial array if no ConsumeMaterial is added', () => {
        const consumeMaterialCollection: IConsumeMaterial[] = [sampleWithRequiredData];
        expectedResult = service.addConsumeMaterialToCollectionIfMissing(consumeMaterialCollection, undefined, null);
        expect(expectedResult).toEqual(consumeMaterialCollection);
      });
    });

    describe('compareConsumeMaterial', () => {
      it('Should return true if both entities are null', () => {
        const entity1 = null;
        const entity2 = null;

        const compareResult = service.compareConsumeMaterial(entity1, entity2);

        expect(compareResult).toEqual(true);
      });

      it('Should return false if one entity is null', () => {
        const entity1 = { id: 123 };
        const entity2 = null;

        const compareResult1 = service.compareConsumeMaterial(entity1, entity2);
        const compareResult2 = service.compareConsumeMaterial(entity2, entity1);

        expect(compareResult1).toEqual(false);
        expect(compareResult2).toEqual(false);
      });

      it('Should return false if primaryKey differs', () => {
        const entity1 = { id: 123 };
        const entity2 = { id: 456 };

        const compareResult1 = service.compareConsumeMaterial(entity1, entity2);
        const compareResult2 = service.compareConsumeMaterial(entity2, entity1);

        expect(compareResult1).toEqual(false);
        expect(compareResult2).toEqual(false);
      });

      it('Should return false if primaryKey matches', () => {
        const entity1 = { id: 123 };
        const entity2 = { id: 123 };

        const compareResult1 = service.compareConsumeMaterial(entity1, entity2);
        const compareResult2 = service.compareConsumeMaterial(entity2, entity1);

        expect(compareResult1).toEqual(true);
        expect(compareResult2).toEqual(true);
      });
    });
  });

  afterEach(() => {
    httpMock.verify();
  });
});
