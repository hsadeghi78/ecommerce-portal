...
type RestOf<T extends IProduct | NewProduct> = Omit<T, 'publishDate' | 'transportDate'> & {
...
export class ProductService {
  ...
  protected convertDateFromClient<T extends IProduct | NewProduct | PartialUpdateProduct>(product: T): RestOf<T> {
    return {
      ...
      transportDate: product.transportDate?.format(DATE_FORMAT) ?? null,
    };
  }
  ...
  protected convertDateFromServer(restProduct: RestProduct): IProduct {
    return {
      ...
      transportDate: restProduct.transportDate ? dayjs(restProduct.transportDate) : undefined,
    };
  }
