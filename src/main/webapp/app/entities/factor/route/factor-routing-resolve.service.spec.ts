...
import factorResolve from './factor-routing-resolve.service';

describe('Factor routing resolve service', () => {
  let mockRouter: Router;
  let mockActivatedRouteSnapshot: ActivatedRouteSnapshot;
  let service: FactorService;
  let resultFactor: IFactor | null | undefined;
  ...
  describe('resolve', () => {
    it('should return IFactor returned by find', () => {
      ...
      TestBed.runInInjectionContext(() => {
        factorResolve(mockActivatedRouteSnapshot).subscribe({
          next(result) {
            resultFactor = result;
          },
        });
    ...
    it('should return null if id is not provided', () => {
      ...
      expect(resultFactor).toEqual(null);
    });

    it('should route to 404 page if data not found in server', () => {
      // GIVEN
      jest.spyOn(service, 'find').mockReturnValue(of(new HttpResponse<IFactor>({ body: null })));
      mockActivatedRouteSnapshot.params = { id: 123 };
